...
def put_request(url, api_key, data=None):
    headers = {'key': api_key}
...
def get_data_credits_balance(api_key: str) -> List:
    url = f"https://console.helium.com/api/v1/organization"
    return get_request(url, api_key).status_code == 200


def create_mqtt_integration(api_key: str, name: str, type: str, endpoint: str, uplink_topic: str, downlink_topic: str) -> Integration:
    url = f"https://console.helium.com/api/v1/integrations"
    body = {
        'name': name,
        'type': type,
        'endpoint': endpoint,
    ...
    return Integration(post_request(url, api_key, body))


def update_device_active_status(api_key: str, active: str, device_id: str) -> Device:
    url = f"https://console.helium.com/api/v1/devices/{device_id}"
    body = {
        'active': active,
    }
    return Device(put_request(url, api_key, body))